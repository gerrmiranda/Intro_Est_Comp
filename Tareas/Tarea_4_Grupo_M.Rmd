...
halley <- function(FUN,FUN1,FUN2,x0,tol=1e-9,maxiter=1000,trace=TRUE){
...
```{r}
ipf <- function(FUN,x0,tol=1e-7,maxiter=1000,trace=TRUE){
    x_new <- FUN(x0)
    iter<-1
    while(abs(x_new-x0) > tol & iter<maxiter){
        x0<-x_new
...
nr <- function(FUN,FUN1,x0,tol=1e-7,maxiter=1000,trace=TRUE){
...
Y creamos una función para comparar los distintos métodos El argumento conFUN es la función que utilizará el método de IPF.

```{r}
comparacion <- function(FUN,FUN1,FUN2,conFUN,x0,tol=1e-5,maxiter=1000,trace=FALSE){
    nr <- nr(FUN,FUN1,x0,tol=tol,maxiter=maxiter,trace=trace)
    ipf <- ipf(conFUN,x0,tol=tol,maxiter=maxiter,trace=trace) 
    halley <- halley(FUN,FUN1,FUN2,x0,tol=tol,maxiter=maxiter,trace=trace)
